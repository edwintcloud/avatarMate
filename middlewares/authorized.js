...
module.exports = function(req, res, next) {
  ...
  if(!req.headers.authorization || req.headers.authorization.split(' ')[0] != 'Bearer') {
    res.status(401);
    next(new Error(`Unauthorized - No header set, please set authorization header with your API key.`));
  }
  ...
  jwt.verify(req.headers.authorization.split(' ')[1], process.env.JWT_SECRET, function(err, decoded) {
    if(err) {
      res.status(401);
      next(new Error(`Unauthorized - Invalid signature, please ensure your API key is valid.`));
    }
